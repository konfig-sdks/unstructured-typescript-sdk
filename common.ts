...
import { readableStreamToString, UnstructuredError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new UnstructuredError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new UnstructuredError(e, e.response?.data)
                }
                if (innerError instanceof UnstructuredError) {
                    // Got "UnstructuredError" from the above try block
                    throw innerError;
                }
